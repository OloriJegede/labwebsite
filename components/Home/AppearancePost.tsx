...
const POSTS_QUERY = `*[
  ...
  && "appearance" in categories[]->slug.current
  ...
  return (
    <div className="bg-[#F9F9F9] px-4 md:px-0">
      <section className="max-w-7xl mx-auto py-14">
        <div className="text-[24px] text-center mb-6">APPEARANCE</div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {validPosts.map((post) => (
            <Post
              key={post._id}
              image={post.mainImage!.asset!.url!}
              category={post.category || "Appearance"}
              date={
                post.publishedAt
        ...
        {validPosts.length === 0 && (
          <div className="text-center text-gray-500 py-8">
            No appearance posts available yet.
          </div>
        )}
      </section>
    </div>
